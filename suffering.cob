	...
	PERFORM VARYING I FROM 2 BY 1 UNTIL I > LAYERCOUNT
		PERFORM VARYING J FROM 1 BY 1 UNTIL J > ELEM OF LAYER_SIZE(I)
			...
			PERFORM VARYING K FROM 1 BY 1 UNTIL K > ELEM OF LAYER_SIZE(I - 1)
				PERFORM FEED-FORWARD-HELPER
				COMPUTE SUMVAL = SUMVAL + FF_HELPER_VAL
			END-PERFORM
			MOVE 1 TO K
			COMPUTE SUMVAL=SUMVAL+ELEM OF WEIGHT(I,J,ELEM OF LAYER_SIZE(I - 1))
			PERFORM SIGMOID
...
FEED-FORWARD-HELPER.
	MOVE ELEM OF OUTPUT_MAT(J,(I - 1)) TO FF_HELPER_VAL.
	COMPUTE FF_HELPER_VAL = FF_HELPER_VAL * ELEM OF WEIGHT(I,J,K).
...
HIDDEN-DELTAS-HELPER.
	...
	MOVE ELEM OF WEIGHT((I + 1),K,J) TO HD_HELPER_WM.
...
MOMENTUM-HELPER-A.
	COMPUTE M_HELPER_A = ALPHA * ELEM OF PREVWEIGHT(I,J,K).
	COMPUTE M_HELPER_A = ELEM OF WEIGHT(I,J,K) - M_HELPER_A.
	MOVE M_HELPER_A TO ELEM OF WEIGHT(I,J,K).
	
	MOVE 0.0 TO M_HELPER_A.

MOMENTUM-HELPER-B.
	MOVE ELEM OF LAYER_SIZE(I - 1) TO Q.
	COMPUTE M_HELPER_B = ALPHA * ELEM OF PREVWEIGHT(I,J,Q).
	COMPUTE M_HELPER_B = ELEM OF WEIGHT(I,J,Q) - M_HELPER_B
	MOVE M_HELPER_B TO ELEM OF WEIGHT(I,J,Q).
...
WEIGHT-HELPER-A.
	...
	MOVE WEIGHT_OLD TO ELEM OF PREVWEIGHT(I,J,K).

	COMPUTE WEIGHT_NEW = ELEM OF WEIGHT(I,J,K) + WEIGHT_OLD.
	MOVE WEIGHT_NEW TO ELEM OF WEIGHT(I,J,K).
...
WEIGHT-HELPER-B.
	...
	MOVE WEIGHT_OLD TO ELEM OF PREVWEIGHT(I,J,(ELEM OF LAYER_SIZE(I - 1))).

	MOVE ELEM OF WEIGHT(I,J,(ELEM OF LAYER_SIZE(I - 1))) TO WEIGHT_NEW.
	COMPUTE WEIGHT_NEW = WEIGHT_NEW + WEIGHT_OLD.
	MOVE WEIGHT_NEW TO ELEM OF WEIGHT(I,J,(ELEM OF LAYER_SIZE(I - 1))).
    ...
    PERFORM VARYING I FROM 1 BY 1 UNTIL I > 4
	    PERFORM VARYING J FROM 1 BY 1 UNTIL J > 3
		    PERFORM VARYING K FROM 1 BY 1 UNTIL K > 4
			    PERFORM RNG
			    MOVE RAND_VAL TO ELEM OF WEIGHT(I,J,K)
			    MOVE 0.0 TO ELEM OF PREVWEIGHT(I,J,K)
		    END-PERFORM
	    END-PERFORM
    END-PERFORM.
    MOVE 1 TO I.
